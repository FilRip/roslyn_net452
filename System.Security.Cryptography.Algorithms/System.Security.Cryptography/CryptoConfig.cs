...
namespace System.Security.Cryptography
{
	public class CryptoConfig
	{
		...
		private static readonly char[] SepArray = new char[1] { '.' };
		...
		public static void AddAlgorithm(Type algorithm, params string[] names)
		{
			...
			{
				throw new ArgumentException(SR.Cryptography_AlgorithmTypesMustBeVisible, "algorithm");
			}
			if (names == null)
			{
		...
		public static object CreateFromName(string name, params object[] args)
		{
			...
			lock (s_InternalSyncObject)
			{
				if (!appNameHT.TryGetValue(name, out value))
				{
					value = null;
				}
			}
			if (value == null && DefaultNameHT.TryGetValue(name, out var value2))
			{
				if (value2 is Type)
				{
					value = (Type)value2;
				...
				{
					value = Type.GetType((string)value2, throwOnError: false, ignoreCase: false);
		...
		public static string MapNameToOID(string name)
		{
			...
			lock (s_InternalSyncObject)
			{
				if (!appOidHT.TryGetValue(name, out value))
				{
					value = null;
				}
			}
		...
		public static byte[] EncodeOID(string str)
		{
			...
			string[] array = str.Split(SepArray);
