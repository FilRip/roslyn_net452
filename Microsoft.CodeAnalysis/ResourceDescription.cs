    ...
    public sealed class ResourceDescription : Cci.IFileReference
    {
        ...
        public ResourceDescription(string resourceName, string? fileName, Func<Stream> dataProvider, bool isPublic, bool isEmbedded, bool checkArgs)
        {
            if (checkArgs)
            {
                ...
                {
                    throw new ArgumentException(Properties.Resources.EmptyOrInvalidResourceName, nameof(resourceName));
                }
                ...
                if (!isEmbedded)
                {
                    ...
                    {
                        throw new ArgumentException(Properties.Resources.EmptyOrInvalidFileName, nameof(fileName));
                    }
                }
            }
        ...
        private sealed class ResourceHashProvider : CryptographicHashProvider
        {
            ...
            internal override ImmutableArray<byte> ComputeHash(HashAlgorithm algorithm)
            {
                try
                {
                    using var stream = _resource.DataProvider() ?? throw new InvalidOperationException(Properties.Resources.ResourceDataProviderShouldReturnNonNullStream);
