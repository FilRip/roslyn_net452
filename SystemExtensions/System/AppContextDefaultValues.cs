...
namespace System
{
    internal static class AppContextDefaultValues
    {
        ...
        private static string GetSwitchValueFromRegistry(string switchName)
        {
            try
            {
                RegistryKey cle = Registry.LocalMachine.OpenSubKey("SOFTWARE\\Microsoft\\.NETFramework\\AppContext", false);
                if (cle != null && cle.GetValue(switchName) != null)
                {
                    return cle.GetValue(switchName).ToString();
                }
                else
                {
