...
#pragma warning restore RS0026 // Do not add multiple public overloads with optional parameters
            ...
            bool minimal)
        {
            ...
            if (minimal)
            {
                ...
                {
                    throw new ArgumentException(Properties.Resources.SyntaxTreeSemanticModelMust);
                }
                else if (positionOpt < 0 || positionOpt > semanticModelOpt.SyntaxTree.Length) // Note: not >= since EOF is allowed.
                {
                    throw new ArgumentOutOfRangeException(Properties.Resources.PositionNotWithinTree);
                }
            }
            else
            {
