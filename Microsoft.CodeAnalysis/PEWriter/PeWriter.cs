    ...
    internal static class PeWriter
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            var peIdProvider = isDeterministic ?
                new Func<IEnumerable<Blob>, BlobContentId>(content => BlobContentId.FromHash(CryptographicHashProvider.ComputeHash(context.Module.PdbChecksumAlgorithm, content))) : // FilRip modified
                null;
            ...
            if (mdWriter.EmitPortableDebugMetadata)
            {
                ...
                Debug.Assert(!isDeterministic || context.Module.PdbChecksumAlgorithm != SourceHashAlgorithm.None);

                var portablePdbIdProvider = (context.Module.PdbChecksumAlgorithm != SourceHashAlgorithm.None) ?
