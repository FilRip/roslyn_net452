...
#pragma warning restore CS0649
        ...
        public new CSharpCompilation AddSyntaxTrees(IEnumerable<SyntaxTree> trees)
        {
            ...
            foreach (var tree in trees.Cast<CSharpSyntaxTree>())
            {
                ...
                {
                    throw new ArgumentException(Properties.Resources.TreeMustHaveARootNodeWith, $"{nameof(trees)}[{i}]");
                }

                if (externalSyntaxTrees.Contains(tree))
                {
                    throw new ArgumentException(Properties.Resources.SyntaxTreeAlreadyPresent, $"{nameof(trees)}[{i}]");
                }

                if (this.IsSubmission && tree.Options.Kind == SourceCodeKind.Regular)
                {
                    throw new ArgumentException(Properties.Resources.SubmissionCanOnlyInclude, $"{nameof(trees)}[{i}]");
                }
            ...
            {
                throw new ArgumentException(Properties.Resources.SubmissionCanHaveAtMostOne, nameof(trees));
            }
        ...
        public new CSharpCompilation RemoveSyntaxTrees(IEnumerable<SyntaxTree> trees)
        {
            ...
            foreach (var tree in trees.Cast<CSharpSyntaxTree>())
            {
                if (!externalSyntaxTrees.Contains(tree))
                {
                    ...
                    {
                        throw new ArgumentException(Properties.Resources.SyntaxTreeFromLoadNoRemoveReplace, $"{nameof(trees)}[{i}]");
                    }

                    throw new ArgumentException(Properties.Resources.SyntaxTreeNotFoundToRemove, $"{nameof(trees)}[{i}]");
                }
        ...
        public new CSharpCompilation ReplaceSyntaxTree(SyntaxTree oldTree, SyntaxTree? newTree)
        {
            ...
            {
                throw new ArgumentException(Properties.Resources.TreeMustHaveARootNodeWith, nameof(newTree));
            }
            ...
            if (!externalSyntaxTrees.Contains(oldTree))
            {
                ...
                {
                    throw new ArgumentException(Properties.Resources.SyntaxTreeFromLoadNoRemoveReplace, nameof(oldTree));
                }

                throw new ArgumentException(Properties.Resources.SyntaxTreeNotFoundToRemove, nameof(oldTree));
            }

            if (externalSyntaxTrees.Contains(newTree))
            {
                throw new ArgumentException(Properties.Resources.SyntaxTreeAlreadyPresent, nameof(newTree));
            }
...
#nullable enable
        ...
        {
            return this.Assembly.GetTypeByMetadataName(fullyQualifiedMetadataName, includeReferences: true, isWellKnownType: false, conflicts: out _);
        }
...
#nullable enable
        ...
        protected override bool IsSymbolAccessibleWithinCore(
            ISymbol symbol,
            ISymbol within,
...
#nullable enable
        ...
        public new SemanticModel GetSemanticModel(SyntaxTree syntaxTree, bool ignoreAccessibility)
        {
            ...
            {
                throw new ArgumentException(Properties.Resources.SyntaxTreeNotFound, nameof(syntaxTree));
            }
        ...
        {
            if (ignoreAccessibility && SimpleProgramNamedTypeSymbol.GetSimpleProgramEntryPoint(this) is not null)
            {
...
#nullable enable
            ...
            void compileMethodBodiesAndDocComments(SyntaxTree? filterTree, TextSpan? filterSpan, BindingDiagnosticBag bindingDiagnostics, CancellationToken cancellationToken)
            {
                MethodCompiler.CompileMethodBodies(
                                ...
                                filterOpt: filterTree is not null ? (s => IsDefinedOrImplementedInSourceTree(s, filterTree, filterSpan)) : null,
                                cancellationToken: cancellationToken);
...
#nullable enable
        ...
        public override bool CompileMethods(
        ...
        public override bool GenerateResourcesAndDocumentationComments(
            CommonPEModuleBuilder moduleBuilder,
            Stream? xmlDocStream,
            ...
            ImmutableArray<INamedTypeSymbol> callingConventionTypes)
        {
            ...
            if (parameterRefKinds.Length != parameterTypes.Length)
            {
                ...
                throw new ArgumentException(string.Format(Properties.Resources.NotSameNumberParameterTypesAndRefKinds, parameterTypes.Length, parameterRefKinds.Length));
            }

            if (returnRefKind == RefKind.Out)
            {
                //'RefKind.Out' is not a valid ref kind for a return type.
                throw new ArgumentException(Properties.Resources.OutIsNotValidForReturn);
            }

            if (callingConvention != SignatureCallingConvention.Unmanaged && !callingConventionTypes.IsDefaultOrEmpty)
            {
                throw new ArgumentException(string.Format(Properties.Resources.CallingConventionTypesRequireUnmanaged, nameof(callingConventionTypes), nameof(callingConvention)));
            }
            ...
            static CustomModifier getCustomModifierForType(INamedTypeSymbol type, CSharpCompilation @this, int index)
            {
                ...
                {
                    throw new ArgumentException(string.Format(Properties.Resources.CallingConventionTypeIsInvalid, type.ToDisplayString()));
                }
        ...
        public override bool ContainsSymbolsWithName(Func<string, bool> predicate, SymbolFilter filter = SymbolFilter.TypeAndMember, CancellationToken cancellationToken = default)
        {
            ...
            {
                throw new ArgumentException(Properties.Resources.NoNoneSearchCriteria, nameof(filter));
            }
