    ...
    public struct MetadataReferenceProperties : IEquatable<MetadataReferenceProperties>
    {
        ...
        public MetadataReferenceProperties(MetadataImageKind kind = MetadataImageKind.Assembly, ImmutableArray<string> aliases = default, bool embedInteropTypes = false)
        {
            ...
            if (kind == MetadataImageKind.Module)
            {
                ...
                {
                    throw new ArgumentException(Properties.Resources.CannotEmbedInteropTypesFromModule, nameof(embedInteropTypes));
                }

                if (!aliases.IsDefaultOrEmpty)
                {
                    throw new ArgumentException(Properties.Resources.CannotAliasModule, nameof(aliases));
                }
            }
            ...
            if (!aliases.IsDefaultOrEmpty)
            {
                foreach (var alias in aliases)
                {
                    ...
                    {
                        throw new ArgumentException(Properties.Resources.InvalidAlias, nameof(aliases));
                    }
                }
            }
