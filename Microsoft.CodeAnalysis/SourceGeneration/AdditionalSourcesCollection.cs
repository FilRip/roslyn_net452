        ...
        public void Add(string hintName, SourceText source)
        {
            ...
            for (int i = 0; i < hintName.Length; i++)
            {
                ...
                if (!UnicodeCharacterUtilities.IsIdentifierPartCharacter(c)
                    ...
                    && c != '}')
                {
                    throw new ArgumentException(string.Format(Properties.Resources.HintNameInvalidChar, c, i), nameof(hintName));
                }
            }
            ...
            {
                throw new ArgumentException(Properties.Resources.HintNameUniquePerGenerator, nameof(hintName));
            }

            if (source.Encoding is null)
            {
                throw new ArgumentException(Properties.Resources.SourceTextRequiresEncoding, nameof(source));
            }
