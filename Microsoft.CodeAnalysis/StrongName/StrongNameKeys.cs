...
namespace Microsoft.CodeAnalysis
{
    public sealed class StrongNameKeys
    {
        ...
        public StrongNameKeys(ImmutableArray<byte> keyPair, ImmutableArray<byte> publicKey, RSAParameters? privateKey, string? keyContainerName, string? keyFilePath, bool hasCounterSignature)
        {
        ...
        internal static StrongNameKeys CreateHelper(ImmutableArray<byte> keyFileContent, string keyFilePath, bool hasCounterSignature)
        {
            ...
            else
            {
                ...
                {
                    throw new IOException(Properties.Resources.InvalidPublicKey);
                }
        ...
        public static StrongNameKeys Create(StrongNameProvider? providerOpt, string? keyFilePath, string? keyContainerName, bool hasCounterSignature, CommonMessageProvider messageProvider)
        {
            ...
            if (providerOpt == null)
            {
                var diagnostic = GetError(keyFilePath, keyContainerName, new CodeAnalysisResourcesLocalizableErrorArgument(nameof(Properties.Resources.AssemblySigningNotSupported)), messageProvider);
