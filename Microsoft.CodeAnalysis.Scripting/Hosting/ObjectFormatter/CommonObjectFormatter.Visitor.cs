    ...
    public abstract partial class CommonObjectFormatter
    {
        private sealed partial class Visitor
        {
            ...
            public string FormatObject(object obj)
            {
                ...
                {
                    return Properties.Resources.StackOverflowWhileEvaluating;
                }
            }
            ...
            private void FormatObjectMembersRecursive(List<FormattedMember> result, object obj, bool includeNonPublic, ref int lengthLimit)
            {
                ...
                foreach (var member in members)
                {
                        ...
                        if (value != null && !VisitedObjects.Contains(value))
                        {
                            if (value is Array array)  // TODO (tomat): n-dim arrays
                            {
                                ...
                                foreach (object item in array)
                                {
                                    ...
                                    FormatObjectRecursive(valueBuilder, item, isRoot: false, debuggerDisplayName: out string name);
                    ...
                    {
                        Builder valueBuilder = MakeMemberBuilder(lengthLimit);
                        FormatObjectRecursive(valueBuilder, value, isRoot: false, debuggerDisplayName: out string name);
            ...
            private void FormatKeyValuePair(Builder result, object obj)
            {
                ...
                object key = type.GetDeclaredProperty("Key").GetValue(obj, new object[0] { });
                object value = type.GetDeclaredProperty("Value").GetValue(obj, new object[0] { });
                string _;
                result.AppendGroupOpening();
