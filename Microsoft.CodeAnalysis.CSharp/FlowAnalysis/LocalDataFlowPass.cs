...
namespace Microsoft.CodeAnalysis.CSharp
{
    ...
    /// </summary>
    public abstract partial class LocalDataFlowPass<TLocalState, TLocalFunctionState> : AbstractFlowPass<TLocalState, TLocalFunctionState>
    ...
    {
        public interface ILocalDataFlowState : ILocalState
        {
            /// <summary>
        ...
        private int DescendThroughTupleRestFields(ref Symbol symbol, int containingSlot, bool forceContainingSlotsToExist)
        {
            if (symbol is TupleElementFieldSymbol fieldSymbol)
            {
                ...
                {
                    if (containingType.GetMembers(NamedTypeSymbol.ValueTupleRestFieldName).FirstOrDefault() is not FieldSymbol restField)
                    {
                        return -1;
                    }
