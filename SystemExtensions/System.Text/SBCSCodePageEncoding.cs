    ...
    internal class SBCSCodePageEncoding : BaseCodePageEncoding
    {
        ...
        protected unsafe override void ReadBestFitTable()
        {
            lock (InternalSyncObject)
            {
                ...
                fixed (byte* ptr = array)
                {
                    ...
                    ushort num;
                    while ((num = *(ushort*)ptr2) != 0)
                    {
                        ptr2 += 2;
                        array2[num] = *(char*)ptr2;
                        ptr2 += 2;
                    }
                    ...
                    while (num3 < 65536)
                    {
                        ...
                        if (b2 > 0)
                        {
                            ...
                            array2[num2++] = _mapBytesToUnicode[b2];
                        }
                        num3++;
                    }
        ...
        public unsafe override int GetByteCount(char* chars, int count, EncoderNLS encoder)
        {
            ...
            while ((c2 = ((encoderFallbackBuffer != null) ? encoderFallbackBufferHelper.InternalGetNextChar() : '\0')) != 0 || chars < ptr)
            {
                ...
                if (_mapUnicodeToBytes[c2] == 0 && c2 != 0)
                {
        ...
        public unsafe override int GetBytes(char* chars, int charCount, byte* bytes, int byteCount, EncoderNLS encoder)
        {
            ...
            if (encoderReplacementFallback != null && encoderReplacementFallback.MaxCharCount == 1)
            {
                byte b = _mapUnicodeToBytes[encoderReplacementFallback.DefaultString[0]];
                if (b != 0)
                {
                    if (c > '\0')
                    {
        ...
        public unsafe override int GetChars(byte* bytes, int byteCount, char* chars, int charCount, DecoderNLS decoder)
        {
            ...
            DecoderFallbackBufferHelper decoderFallbackBufferHelper = new((decoder != null) ? decoder.FallbackBuffer : base.DecoderFallback.CreateFallbackBuffer());
            while (bytes < ptr)
            {
