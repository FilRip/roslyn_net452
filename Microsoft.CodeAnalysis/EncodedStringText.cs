...
#nullable restore
        ...
        public static byte[] TryGetByteArrayFromStream(Stream data)
        {
            ...
            if (data is MemoryStream memoryStream && TryGetByteArrayFromMemoryStream(memoryStream, out byte[] buffer))
            {
                return buffer;
            }
            ...
            if (data is FileStream &&
                TryGetByteArrayFromFileStream(data as FileStream, out buffer))
            {
                return buffer;
            }
        ...
        // FilRip : Remove methods by above
