...
namespace Microsoft.CodeAnalysis
{
    ...
    public struct GeneratorInitializationContext
    {
        ...
        private static void CheckIsEmpty<T>(T x, string? typeName = null) where T : class?
        {
            if (x is object)
            {
                throw new InvalidOperationException(string.Format(Properties.Resources.Single_type_per_generator_0, typeName ?? typeof(T).Name));
            }
        }
    }
    ...
    {
        public GeneratorEditContext(AdditionalSourcesCollection sources, CancellationToken cancellationToken = default)
        {
