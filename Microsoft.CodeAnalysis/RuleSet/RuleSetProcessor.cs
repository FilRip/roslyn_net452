                ...
                if (childNode.Name == RulesNodeName)
                {
                    ...
                    foreach (var rule in rules)
                    {
                        ...
                        if (specificOptions.TryGetValue(ruleId, out ReportDiagnostic existingAction))
                        {
                            ...
                            {
                                throw new InvalidRuleSetException(string.Format(Properties.Resources.RuleSetHasDuplicateRules, ruleId, existingAction, action));
                            }
                        }
                        else
                        {
...
#nullable disable
        ...
        private static ReportDiagnostic ReadAction(XElement node, bool allowDefault)
        {
            ...
            throw new InvalidRuleSetException(string.Format(Properties.Resources.RuleSetBadAttributeValue, RuleActionAttributeName, action));
        }
