...
namespace Microsoft.CodeAnalysis.CSharp.Syntax.InternalSyntax
{
    ...
    internal partial class LanguageParser : SyntaxParser
    {
        ...
        private static bool IsPossibleStartOfTypeDeclaration(SyntaxKind kind)
        {
            return kind switch
            {
                SyntaxKind.EnumKeyword or SyntaxKind.DelegateKeyword or SyntaxKind.ClassKeyword or SyntaxKind.InterfaceKeyword or SyntaxKind.StructKeyword or SyntaxKind.AbstractKeyword or SyntaxKind.InternalKeyword or SyntaxKind.NewKeyword or SyntaxKind.PrivateKeyword or SyntaxKind.ProtectedKeyword or SyntaxKind.PublicKeyword or SyntaxKind.SealedKeyword or SyntaxKind.StaticKeyword or SyntaxKind.UnsafeKeyword or SyntaxKind.OpenBracketToken => true,
                _ => false,
            };
        }
        ...
        private void ParseNamespaceBody(ref SyntaxToken openBrace, ref NamespaceBodyBuilder body, ref SyntaxListBuilder initialBadNodes, SyntaxKind parentKind)
        {
            ...
            try
            {
                    ...
                    switch (this.CurrentToken.Kind)
                    {
                        ...
                        case SyntaxKind.ExternKeyword:
                            if (isGlobal && !ScanExternAliasDirective())
                            {
                                ...
                                DefaultCase(ref openBrace, ref body, ref initialBadNodes);
                                break;
                            }
                            else
                            {
                ...
                void DefaultCase(ref SyntaxToken openBrace, ref NamespaceBodyBuilder body, ref SyntaxListBuilder initialBadNodes)
                {
...
#pragma warning restore S125 // Sections of code should not be commented out
            ...
            try
            {
                ...
                if (this.ScanType() == EScanType.NotType)
                {
                    return false;
                }
        ...
        private MemberDeclarationSyntax ParseMemberDeclarationOrStatementCore(SyntaxKind parentKind)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    bool loop = true;
                    ExplicitInterfaceSpecifierSyntax explicitInterfaceOpt = null;
                    SyntaxToken identifierOrThisOpt = null;
        ...
        private MemberDeclarationSyntax ParseMemberDeclarationCore(SyntaxKind parentKind)
        {
            ...
            try
            {
                ...
                try
                {
                    // Check for misplaced modifiers.  if we see any, then consider this member
                    // terminated and restart parsing.
                    if (IsMisplacedModifier(modifiers, attributes, type, out MemberDeclarationSyntax result))
                    {
                        return result;
                    }
                    ExplicitInterfaceSpecifierSyntax explicitInterfaceOpt = null;
...
#pragma warning restore S125 // Sections of code should not be commented out
            ...
            TypeParameterListSyntax typeParameterList)
        {
            ...
            try
            {
                ...
                this.ParseBlockAndExpressionBodiesWithSemicolon(out BlockSyntax blockBody, out ArrowExpressionClauseSyntax expressionBody, out SyntaxToken semicolon);
            ...
            TypeSyntax type)
        {
            ...
            if (opToken.Kind == SyntaxKind.GreaterThanToken && tk.Kind == SyntaxKind.GreaterThanToken && (opToken.GetTrailingTriviaWidth() == 0 && tk.GetLeadingTriviaWidth() == 0))
            {
                // no trailing trivia and no leading trivia
                SyntaxToken opToken2 = this.EatToken();
...
#pragma warning restore S125 // Sections of code should not be commented out
            ...
            where TNode : CSharpSyntaxNode
        {
            ...
            var action = this.SkipBadListTokensWithExpectedKindHelper(list.UnderlyingBuilder, isNotExpectedFunction, abortFunction, expected, out GreenNode trailingTrivia);
            ...
            out GreenNode trailingTrivia)
        {
            ...
            {
                var action = SkipBadTokensWithExpectedKind(isNotExpectedFunction, abortFunction, expected, out GreenNode lastItemTrailingTrivia);
        ...
        private AccessorDeclarationSyntax ParseAccessorDeclaration(bool isEvent)
        {
            ...
            SyntaxListBuilder accMods = _pool.Allocate();
            try
            {
                ...
                if (!isEvent && accMods != null && accMods.Count > 0)
                {
                    accMods[0] = CheckFeatureAvailability(accMods[0], MessageID.IDS_FeaturePropertyAccessorMods);
                }

                SyntaxToken accessorName = this.EatToken(SyntaxKind.IdentifierToken,
                ...
                return _syntaxFactory.AccessorDeclaration(
                    accessorKind, accAttrs, (accMods == null ? null : accMods.ToList()), accessorName,
        ...
        internal ParameterListSyntax ParseParenthesizedParameterList()
        {
            ...
            try
            {
                ...
                this.ParseParameterList(out SyntaxToken open, parameters, out SyntaxToken close, openKind, closeKind);
            ...
            SyntaxKind parentKind)
        {
            ...
            try
            {
                this.ParseVariableDeclarators(type, EVariable.Fixed, variables, parentKind);
            ...
            TypeSyntax type)
        {
            ...
            if (explicitInterfaceOpt != null && this.CurrentToken.Kind != SyntaxKind.OpenBraceToken && this.CurrentToken.Kind != SyntaxKind.SemicolonToken)
            {
                ...
                var missingIdentifier = identifierOrThisOpt ?? CreateMissingIdentifierToken();
...
#pragma warning restore S125 // Sections of code should not be commented out
        ...
        private void ParseVariableDeclarators(TypeSyntax type, EVariable flags, SeparatedSyntaxListBuilder<VariableDeclaratorSyntax> variables, SyntaxKind parentKind)
        {
        ...
        private static EVariable GetOriginalVariableFlags(Syntax.VariableDeclaratorSyntax old)
        {
            var parent = GetOldParent(old);
            var mods = GetOriginalModifiers(parent);
            EVariable flags = default;
            if (mods.Any(SyntaxKind.FixedKeyword))
            {
                flags |= EVariable.Fixed;
            }

            if (mods.Any(SyntaxKind.ConstKeyword))
            {
                flags |= EVariable.Const;
            }

            if (parent != null && (parent.Kind() == SyntaxKind.VariableDeclaration || parent.Kind() == SyntaxKind.LocalDeclarationStatement))
            {
                flags |= EVariable.Local;
            }

            return flags;
        }

        private static bool CanReuseVariableDeclarator(Syntax.VariableDeclaratorSyntax old, EVariable flags, bool isFirst)
        {
            if (old == null)
            {
                return false;
...
#pragma warning restore S125 // Sections of code should not be commented out
            ...
            bool isFixed = (flags & EVariable.Fixed) != 0;
            bool isConst = (flags & EVariable.Const) != 0;
            bool isLocal = (flags & EVariable.Local) != 0;
...
#pragma warning restore S125 // Sections of code should not be commented out
        ...
        private ExpressionSyntax ParseVariableInitializer()
        {
            return this.CurrentToken.Kind switch
            {
                SyntaxKind.OpenBraceToken => this.ParseArrayInitializer(),
                _ => this.ParseExpressionCore(),
            };
        }
        ...
        private FieldDeclarationSyntax ParseConstantFieldDeclaration(SyntaxList<AttributeListSyntax> attributes, SyntaxListBuilder modifiers, SyntaxKind parentKind)
        {
            ...
            try
            {
                this.ParseVariableDeclarators(type, EVariable.Const, variables, parentKind);
                var semicolon = this.EatToken(SyntaxKind.SemicolonToken);
                return _syntaxFactory.FieldDeclaration(
                    attributes,
                    modifiers.ToList(),
        ...
        private IdentifierNameSyntax ParseIdentifierName(ErrorCode code = ErrorCode.ERR_IdentifierExpected)
        {
            if (this.IsIncrementalAndFactoryContextMatches && this.CurrentNodeKind == SyntaxKind.IdentifierName &&
                !SyntaxFacts.IsContextualKeyword(((CSharp.Syntax.IdentifierNameSyntax)this.CurrentNode).Identifier.Kind()))
            {
                return (IdentifierNameSyntax)this.EatNode();
            }

            SyntaxToken tk = ParseIdentifierToken(code);
...
#pragma warning restore S125 // Sections of code should not be commented out
        ...
        {
            if (this.CurrentToken.ContextualKind == SyntaxKind.PartialKeyword &&
                (this.IsPartialType() || this.IsPartialMember()))
            {
                return true;
            }
        ...
        private SimpleNameSyntax ParseSimpleName(NameOptions options = NameOptions.None)
        {
            ...
            if (this.CurrentToken.Kind == SyntaxKind.LessThanToken)
            {
                ...
                if (kind == ScanTypeArgumentListKind.DefiniteTypeArgumentList || (kind == ScanTypeArgumentListKind.PossibleTypeArgumentList && (options & NameOptions.InTypeList) != 0))
                {
                    ...
                    this.ParseTypeArgumentList(out SyntaxToken open, types, out SyntaxToken close);
        ...
        private ScanTypeArgumentListKind ScanTypeArgumentList(NameOptions options)
        {
            ...
            EScanType possibleTypeArgumentFlags = ScanPossibleTypeArgumentList(
                ref lastTokenOfList, out bool isDefinitelyTypeArgumentList);

            if (possibleTypeArgumentFlags == EScanType.NotType)
            {
                return ScanTypeArgumentListKind.NotTypeArgumentList;
            }
        ...
        private EScanType ScanPossibleTypeArgumentList(
            ref SyntaxToken lastTokenOfList, out bool isDefinitelyTypeArgumentList)
        {
            ...
            if (this.CurrentToken.Kind == SyntaxKind.LessThanToken)
            {
                EScanType result = EScanType.GenericTypeOrExpression;
                ...
                do
                {
                    ...
                    {
                        case EScanType.NotType:
                            lastTokenOfList = null;
                            return EScanType.NotType;

                        case EScanType.MustBeType:
                            ...
                            result = EScanType.GenericTypeOrMethod;
                            break;
...
#pragma warning restore S125 // Sections of code should not be commented out
                        ...
                        case EScanType.GenericTypeOrExpression:
                        ...
                        case EScanType.GenericTypeOrMethod:
                            result = EScanType.GenericTypeOrMethod;
                            break;
                    }
                }
            ...
            return EScanType.NonGenericTypeOrExpression;
        }
...
#pragma warning restore S125 // Sections of code should not be commented out
        ...
        private EScanType ScanType(bool forPattern = false)
        {
            return ScanType(out _, forPattern);
        }

        private EScanType ScanType(out SyntaxToken lastTokenOfType, bool forPattern = false)
        {
            return ScanType(forPattern ? ParseTypeMode.DefinitePattern : ParseTypeMode.Normal, out lastTokenOfType);
        }

        private EScanType ScanType(ParseTypeMode mode, out SyntaxToken lastTokenOfType)
        {
            EScanType result;
            ...
            if (this.CurrentToken.Kind is SyntaxKind.IdentifierToken or SyntaxKind.ColonColonToken)
            {
                ...
                {
                    result = EScanType.NonGenericTypeOrExpression;
                ...
                if (isAlias)
                {
                    result = EScanType.AliasQualifiedName;
                }
            }
            ...
            else if (this.CurrentToken.Kind == SyntaxKind.OpenParenToken)
            {
                ...
                if (result == EScanType.NotType || mode == ParseTypeMode.DefinitePattern && this.CurrentToken.Kind != SyntaxKind.OpenBracketToken)
                {
                ...
                switch (this.CurrentToken.Kind)
                {
                    ...
                    case SyntaxKind.AsteriskToken
                        ...
                        switch (mode)
                        {
                            ...
                            default:
                                lastTokenOfType = this.EatToken();
                                if (result == EScanType.GenericTypeOrExpression || result == EScanType.NonGenericTypeOrExpression)
                                {
                                    result = EScanType.PointerOrMultiplication;
                                }
                                else if (result == EScanType.GenericTypeOrMethod)
                                {
                                    result = EScanType.MustBeType;
                                }
                                break;
        ...
        private EScanType ScanNamedTypePart(out SyntaxToken lastTokenOfType)
        {
        ...
        /// </summary>
        private EScanType ScanTupleType(out SyntaxToken lastTokenOfType)
        {
            var tupleElementType = ScanType(out _);
            if (tupleElementType != EScanType.NotType)
            {
                if (IsTrueIdentifier())
                {
                    this.EatToken();
...
#nullable enable
        private EScanType ScanFunctionPointerType(out SyntaxToken lastTokenOfType)
        {
        ...
        private SyntaxToken EatNullableQualifierIfApplicable(ParseTypeMode mode)
        {
            ...
            try
            {
                ...
                bool canFollowNullableType()
                {
                    return mode switch
                    {
                        ParseTypeMode.AfterIs or ParseTypeMode.DefinitePattern or ParseTypeMode.AsExpression => !CanStartExpression(),// These contexts might be a type that is at the end of an expression.
                        ...
                        ParseTypeMode.NewExpression => this.CurrentToken.Kind == SyntaxKind.OpenParenToken ||   // ctor parameters
                                                        this.CurrentToken.Kind == SyntaxKind.OpenBracketToken ||   // array type
                                                        this.CurrentToken.Kind == SyntaxKind.OpenBraceToken,// A nullable qualifier is permitted as part of the type in a `new` expression.
...
#pragma warning restore S125 // Sections of code should not be commented out
            ...
            try
            {
                ...
                if (st == EScanType.MustBeType && this.CurrentToken.Kind != SyntaxKind.DotToken && this.CurrentToken.Kind != SyntaxKind.OpenParenToken)
                {
                    return true;
                }

                if (st == EScanType.NotType || this.CurrentToken.Kind != SyntaxKind.IdentifierToken)
                {
                    return false;
                }
                ...
                {
                    if (st == EScanType.PointerOrMultiplication)
                    {
                        return false;
                    }
                    else if (st == EScanType.NullableType)
                    {
...
#pragma warning restore S125 // Sections of code should not be commented out
        ...
        private bool IsPossibleNewExpression()
        {
            ...
            try
            {
                ...
                return !IsPossibleMemberName() || st == EScanType.NotType;
            }
            finally
            {
                this.Reset(ref resetPoint);
        ...
        private bool IsPossibleStatement(bool acceptAccessibilityMods)
        {
            ...
            return tk switch
            {
                SyntaxKind.FixedKeyword or SyntaxKind.BreakKeyword or SyntaxKind.ContinueKeyword or SyntaxKind.TryKeyword or SyntaxKind.CheckedKeyword or SyntaxKind.UncheckedKeyword or SyntaxKind.ConstKeyword or SyntaxKind.DoKeyword or SyntaxKind.ForKeyword or SyntaxKind.ForEachKeyword or SyntaxKind.GotoKeyword or SyntaxKind.IfKeyword or SyntaxKind.ElseKeyword or SyntaxKind.LockKeyword or SyntaxKind.ReturnKeyword or SyntaxKind.SwitchKeyword or SyntaxKind.ThrowKeyword or SyntaxKind.UnsafeKeyword or SyntaxKind.UsingKeyword or SyntaxKind.WhileKeyword or SyntaxKind.OpenBraceToken or SyntaxKind.SemicolonToken or SyntaxKind.StaticKeyword or SyntaxKind.ReadOnlyKeyword or SyntaxKind.VolatileKeyword or SyntaxKind.RefKeyword or SyntaxKind.ExternKeyword or SyntaxKind.OpenBracketToken => true,
                SyntaxKind.IdentifierToken => IsTrueIdentifier(),
                SyntaxKind.CatchKeyword or SyntaxKind.FinallyKeyword => !_isInTry,
                // Accessibility modifiers are not legal in a statement,
                // but a common mistake for local functions. Parse to give a
                // better error message.
                SyntaxKind.PublicKeyword or SyntaxKind.InternalKeyword or SyntaxKind.ProtectedKeyword or SyntaxKind.PrivateKeyword => acceptAccessibilityMods,
...
|| IsPossibleExpression(),
        ...
        private StatementSyntax ParseForOrForEachStatement(SyntaxList<AttributeListSyntax> attributes)
        {
            ...
            try
            {
                ...
                if (this.EatToken().Kind == SyntaxKind.OpenParenToken &&
                    this.ScanType() != EScanType.NotType &&
        ...
        private bool IsPossibleDeclarationExpression(ParseTypeMode mode, bool permitTupleDesignation)
        {
            ...
            try
            {
                ...
                if (ScanType(mode, out SyntaxToken lastTokenOfType) == EScanType.NotType)
                {
                    return false;
                }
                ...
                return mode switch
                {
                    ...
                    ParseTypeMode.AfterTupleComma => this.CurrentToken.Kind == SyntaxKind.CommaToken || this.CurrentToken.Kind == SyntaxKind.CloseParenToken,
        ...
        private bool IsUsingStatementVariableDeclaration(EScanType st)
        {

            bool condition1 = st == EScanType.MustBeType && this.CurrentToken.Kind != SyntaxKind.DotToken;
            bool condition2 = st != EScanType.NotType && this.CurrentToken.Kind == SyntaxKind.IdentifierToken;
            bool condition3 = st == EScanType.NonGenericTypeOrExpression || this.PeekToken(1).Kind == SyntaxKind.EqualsToken;
        ...
        private StatementSyntax ParseLocalDeclarationStatement(SyntaxList<AttributeListSyntax> attributes)
        {
            ...
            try
            {
                ...
                if (canParseAsLocalFunction &&
                    attributes.Count == 0 && mods.Count > 0 && IsAccessibilityModifier(((SyntaxToken)mods[0]).ContextualKind))
                {
            ...
            out LocalFunctionStatementSyntax localFunction)
        {
            ...
            EVariable flags = EVariable.Local;
        ...
        private bool IsEndOfDeclarationClause()
        {
            return this.CurrentToken.Kind switch
            {
                SyntaxKind.SemicolonToken or SyntaxKind.CloseParenToken or SyntaxKind.ColonToken => true,
                _ => false,
            };
        }
        ...
        private void ParseDeclarationModifiers(SyntaxListBuilder list)
        {
            ...
            while (IsDeclarationModifier(k = this.CurrentToken.ContextualKind) || IsAdditionalLocalFunctionModifier(k))
            {
                ...
                if (k == SyntaxKind.AsyncKeyword)
                {
                    ...
                    bool invalid = !IsPossibleStartOfTypeDeclaration(this.EatToken().Kind) &&
                        !IsDeclarationModifier(this.CurrentToken.Kind) && !IsAdditionalLocalFunctionModifier(this.CurrentToken.Kind) &&
                        (ScanType() == EScanType.NotType || this.CurrentToken.Kind != SyntaxKind.IdentifierToken);
        ...
        private bool IsPossibleExpression(bool allowBinaryExpressions, bool allowAssignmentExpressions, bool allowAttributes)
        {
            ...
            return tk switch
            {
                ...
                _ => (IsPredefinedType(tk) && tk != SyntaxKind.VoidKeyword)
        ...
        private static Precedence GetPrecedence(SyntaxKind op)
        {
            return op switch
            {
                ...
                SyntaxKind.LeftShiftExpression or SyntaxKind.RightShiftExpression => Precedence.Shift,
                SyntaxKind.AddExpression or SyntaxKind.SubtractExpression => Precedence.Additive,
                SyntaxKind.MultiplyExpression or SyntaxKind.DivideExpression or SyntaxKind.ModuloExpression => Precedence.Mutiplicative,
                ...
                SyntaxKind.AliasQualifiedName or SyntaxKind.AnonymousObjectCreationExpression or SyntaxKind.ArgListExpression or SyntaxKind.ArrayCreationExpression or SyntaxKind.BaseExpression or SyntaxKind.CharacterLiteralExpression or SyntaxKind.ConditionalAccessExpression or SyntaxKind.DeclarationExpression or SyntaxKind.DefaultExpression or SyntaxKind.DefaultLiteralExpression or SyntaxKind.ElementAccessExpression or SyntaxKind.FalseLiteralExpression or SyntaxKind.GenericName or SyntaxKind.IdentifierName or SyntaxKind.ImplicitArrayCreationExpression or SyntaxKind.ImplicitStackAllocArrayCreationExpression or SyntaxKind.ImplicitObjectCreationExpression or SyntaxKind.InterpolatedStringExpression or SyntaxKind.InvocationExpression or SyntaxKind.NullLiteralExpression or SyntaxKind.NumericLiteralExpression or SyntaxKind.ObjectCreationExpression or SyntaxKind.ParenthesizedExpression or SyntaxKind.PointerMemberAccessExpression or SyntaxKind.PostDecrementExpression or SyntaxKind.PostIncrementExpression or SyntaxKind.PredefinedType or SyntaxKind.RefExpression or SyntaxKind.SimpleMemberAccessExpression or SyntaxKind.StackAllocArrayCreationExpression or SyntaxKind.StringLiteralExpression or SyntaxKind.SuppressNullableWarningExpression or SyntaxKind.ThisExpression or SyntaxKind.TrueLiteralExpression or SyntaxKind.TupleExpression => Precedence.Primary,
        ...
        private ExpressionSyntax ParseExpressionContinued(ExpressionSyntax leftOperand, Precedence precedence)
        {
            while (true)
            {
                ...
                if (tk == SyntaxKind.GreaterThanToken
                    ...
                    && (this.CurrentToken.GetTrailingTriviaWidth() == 0 && this.PeekToken(1).GetLeadingTriviaWidth() == 0))
                {
        ...
        private ExpressionSyntax ParseCastOrParenExpressionOrLambdaOrTuple(Precedence precedence)
        {
            ...
            try
            {
                ...
                if (this.ScanCast() && !IsCurrentTokenQueryKeywordInQuery())
                {
...
#pragma warning disable S125 // Sections of code should not be commented out
                ...
                // 4) (G::Goo)bar
#pragma warning restore S125 // Sections of code should not be commented out
                EScanType.PointerOrMultiplication or EScanType.NullableType or EScanType.MustBeType or EScanType.AliasQualifiedName => !forPattern || this.CurrentToken.Kind switch
                {
                  ...
                  // following a cast.
                EScanType.GenericTypeOrMethod or EScanType.GenericTypeOrExpression or EScanType.NonGenericTypeOrExpression or EScanType.TupleType => CanFollowCast(this.CurrentToken.Kind),// check for ambiguous type or expression followed by disambiguating token.  i.e.
                                                                                                                                                                                                           //
...
#pragma warning restore S125 // Sections of code should not be commented out
        ...
        private bool IsImplicitObjectCreation()
        {
            ...
            try
            {
                ...
                EScanType scanTypeFlags = ScanTupleType(out _);
                if (scanTypeFlags != EScanType.NotType)
                {
                    switch (this.CurrentToken.Kind)
                    {
...
#nullable disable
        ...
        private InitializerExpressionSyntax ParseObjectOrCollectionInitializer()
        {
            ...
            try
            {
                this.ParseObjectOrCollectionInitializerMembers(ref openBrace, initializers, out bool isObjectInitializer);
        ...
        private InitializerExpressionSyntax ParseComplexElementInitializer()
        {
            ...
            try
            {
                this.ParseExpressionsForComplexElementInitializer(ref openBrace, initializers, out DiagnosticInfo closeBraceError);
                var closeBrace = this.EatToken(SyntaxKind.CloseBraceToken);
        ...
        private bool IsPossibleLambdaParameter()
        {
            return this.CurrentToken.Kind switch
            {
                SyntaxKind.ParamsKeyword or SyntaxKind.RefKeyword or SyntaxKind.OutKeyword or SyntaxKind.InKeyword or SyntaxKind.OpenParenToken or SyntaxKind.OpenBracketToken => true,
                SyntaxKind.IdentifierToken => this.IsTrueIdentifier(),
...
#pragma warning disable S125 // Sections of code should not be commented out
                if (mayBeVariableDeclaration &&
                    (pk2 == SyntaxKind.SemicolonToken ||    // from x;
...
#pragma warning restore S125 // Sections of code should not be commented out
            ...
            try
            {
                ...
                EScanType isType = this.ScanType();
                if (isType != EScanType.NotType && (this.CurrentToken.Kind == SyntaxKind.IdentifierToken || this.CurrentToken.Kind == SyntaxKind.InKeyword))
                {
                    return true;
                }
            }
        ...
        private QueryBodySyntax ParseQueryBody()
        {
            ...
            try
            {
                ...
                selectOrGroupBy = this.CurrentToken.ContextualKind switch
                {
