...
namespace Microsoft.CodeAnalysis
{
    public abstract class CommandLineParser
    {
        ...
        protected abstract CommandLineArguments CommonParse(IEnumerable<string> args, string baseDirectory, string? sdkDirectory, string? additionalReferenceDirectories);
        ...
        public static bool TryParseOption(string arg, [NotNullWhen(true)] out string? name, out string? value)
        {
            if (!IsOption(arg))
            {
                name = null;
        ...
        public ErrorLogOptions? ParseErrorLogOptions(
            string arg,
            ...
            out string? errorMessage)
        {
            ...
            foreach (var arg in args)
            {
                if (isClientArgsOption(arg, "keepalive", out bool hasValue, out string? value))
                {
                    ...
                    {
                        errorMessage = Properties.Resources.MissingKeepAlive;
                        return false;
                    }
                    ...
                    if (int.TryParse(value, out int intValue))
                    {
                        if (intValue < -1)
                        {
                            errorMessage = Properties.Resources.KeepAliveIsTooSmall;
                            return false;
                        }
                    ...
                    {
                        errorMessage = Properties.Resources.KeepAliveIsNotAnInteger;
                        return false;
                    }
                    continue;
                }
                ...
                if (isClientArgsOption(arg, "shared", out hasValue, out value))
                {
                    if (hasValue)
                    {
                        if (string.IsNullOrEmpty(value))
                        {
                            errorMessage = Properties.Resources.SharedArgumentMissing;
                            return false;
                        }
            ...
            {
                errorMessage = Properties.Resources.KeepAliveWithoutShared;
                return false;
            }
            else
            {
        ...
        internal static string MismatchedVersionErrorText => Properties.Resources.MismatchedVersion;
        ...
        protected abstract void GenerateErrorForNoFilesFoundInRecurse(string path, IList<Diagnostic> errors);
