...
namespace Microsoft.CodeAnalysis.Diagnostics
{
    internal static class DiagnosticAnalysisContextHelpers
    {
        ...
        internal static void VerifyArguments(Diagnostic diagnostic, Compilation? compilation, Func<Diagnostic, bool> isSupportedDiagnostic)
        {
            ...
            {
                throw new ArgumentException(string.Format(Properties.Resources.UnsupportedDiagnosticReported, diagnostic.Id), nameof(diagnostic));
            }
            ...
            if (!UnicodeCharacterUtilities.IsValidIdentifier(diagnostic.Id))
            {
                ...
                throw new ArgumentException(string.Format(Properties.Resources.InvalidDiagnosticIdReported, diagnostic.Id), nameof(diagnostic));
            }
        }
        ...
        internal static void VerifyDiagnosticLocationsInCompilation(Diagnostic diagnostic, Compilation compilation)
        {
            ...
            {
                foreach (var _ in diagnostic.AdditionalLocations)
                {
                    VerifyDiagnosticLocationInCompilation(diagnostic.Id, diagnostic.Location, compilation);
                }
            }
...
#nullable restore
            if (!compilation.ContainsSyntaxTree(location.SourceTree))
            {
                ...
                throw new ArgumentException(string.Format(Properties.Resources.InvalidDiagnosticLocationReported, id, location.SourceTree.FilePath), "diagnostic");
            }

            if (location.SourceSpan.End > location.SourceTree.Length)
            {
                // Disallow diagnostics with source locations outside this compilation.
                throw new ArgumentException(string.Format(Properties.Resources.InvalidDiagnosticSpanReported, id, location.SourceSpan, location.SourceTree.FilePath), "diagnostic");
            }
        }
        ...
        private static void VerifySymbolKinds(ImmutableArray<SymbolKind> symbolKinds)
        {
            ...
            {
                throw new ArgumentException(Properties.Resources.ArgumentCannotBeEmpty, nameof(symbolKinds));
            }
        }
            ...
            where TLanguageKindEnum : struct
        {
            ...
            {
                throw new ArgumentException(Properties.Resources.ArgumentCannotBeEmpty, nameof(syntaxKinds));
            }
        }
        ...
        private static void VerifyOperationKinds(ImmutableArray<OperationKind> operationKinds)
        {
            ...
            {
                throw new ArgumentException(Properties.Resources.ArgumentCannotBeEmpty, nameof(operationKinds));
            }
        }
