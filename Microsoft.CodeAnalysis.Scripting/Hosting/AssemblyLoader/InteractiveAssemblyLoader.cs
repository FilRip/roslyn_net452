...
namespace Microsoft.CodeAnalysis.Scripting.Hosting
{
    ...
    public sealed partial class InteractiveAssemblyLoader : IDisposable
    {
        ...
        public void RegisterDependency(AssemblyIdentity dependency, string path)
        {
            ...
            {
                throw new ArgumentException(Properties.Resources.AbsolutePathExpected, nameof(path));
            }
        ...
        private void RegisterLoadedAssemblySimpleNameNoLock(Assembly assembly, string locationOpt)
        {
            ...
            if (_loadedAssembliesBySimpleName.TryGetValue(identity.Name, out List<LoadedAssemblyInfo> existingInfos))
            {
        ...
        private void RegisterDependencyNoLock(AssemblyIdentityAndLocation dependency)
        {
            ...
            if (_dependenciesWithLocationBySimpleName.TryGetValue(simpleName, out List<AssemblyIdentityAndLocation> sameSimpleNameAssemblyIdentities))
            {
        ...
        internal Assembly ResolveAssembly(string assemblyDisplayName, Assembly requestingAssemblyOpt)
        {
            ...
            lock (_referencesLock)
            {
                if (requestingAssemblyOpt != null &&
                    _assembliesLoadedFromLocation.TryGetValue(requestingAssemblyOpt, out LoadedAssembly loadedAssembly))
                {
        ...
        internal Assembly ResolveAssembly(AssemblyIdentity identity, string loadDirectoryOpt)
        {
            ...
            if (loadDirectoryOpt != null)
            {
                ...
                lock (_referencesLock)
                {
                    ...
                    if (_loadedAssembliesBySimpleName.TryGetValue(identity.Name, out List<LoadedAssemblyInfo> loadedInfos))
                    {
                ...
                if (assemblyFilePathOpt != null)
                {
                    ...
                    {
                        if (TryReadMvid(assemblyFilePathOpt, out Guid mvid) &&
                        ...
                        throw new InteractiveAssemblyLoaderException(
                            string.Format(null, Properties.Resources.AssemblyAlreadyLoaded,
                            identity.Name,
                            identity.Version,
                    ...
                    if (!conflictingLoadedAssemblyOpt.IsDefault)
                    {
                        // error: attempt to load an assembly with the same identity as already loaded assembly but different content
                        throw new InteractiveAssemblyLoaderException(
                            string.Format(null, Properties.Resources.AssemblyAlreadyLoadedNotSigned,
                            identity.Name,
        ...
        private Assembly TryGetAssemblyLoadedFromPath(AssemblyIdentity identity, string directory)
        {
            ...
            foreach (var extension in RuntimeMetadataReferenceResolver.AssemblyExtensions)
            {
                if (_assembliesLoadedFromLocationByFullPath.TryGetValue(pathWithoutExtension + extension, out AssemblyAndLocation assemblyAndLocation) &&
        ...
        private Assembly GetOrLoadKnownAssembly(AssemblyIdentity identity)
        {
            ...
            lock (_referencesLock)
            {
                ...
                if (_loadedAssembliesBySimpleName.TryGetValue(identity.Name, out List<LoadedAssemblyInfo> infos))
                {
                ...
                // names:
                if (_dependenciesWithLocationBySimpleName.TryGetValue(identity.Name, out List<AssemblyIdentityAndLocation> sameSimpleNameIdentities))
                {
                    ...
                    if (identityAndLocation.Identity != null)
                    {
                        ...
                        if (_assembliesLoadedFromLocationByFullPath.TryGetValue(assemblyFileToLoad, out AssemblyAndLocation assemblyAndLocation))
                        {
                            return assemblyAndLocation.Assembly;
                        }
                    }
        ...
        private AssemblyAndLocation ShadowCopyAndLoadDependency(string originalPath)
        {
            ...
            lock (_referencesLock)
            {
                ...
                if (_assembliesLoadedFromLocation.TryGetValue(assemblyAndLocation.Assembly, out LoadedAssembly loadedAssembly))
                {
