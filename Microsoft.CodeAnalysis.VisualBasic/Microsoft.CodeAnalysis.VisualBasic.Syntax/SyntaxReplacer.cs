...
{
	internal class SyntaxReplacer
	{
		private class Replacer<TNode> : VisualBasicSyntaxRewriter where TNode : SyntaxNode
		{
			...
			private readonly bool _visitStructuredTrivia;
			...
			public override bool VisitIntoStructuredTrivia => _visitStructuredTrivia;
			...
			public Replacer(IEnumerable<TNode> nodes, Func<TNode, TNode, SyntaxNode> computeReplacementNode, IEnumerable<SyntaxToken> tokens, Func<SyntaxToken, SyntaxToken, SyntaxToken> computeReplacementToken, IEnumerable<SyntaxTrivia> trivia, Func<SyntaxTrivia, SyntaxTrivia, SyntaxTrivia> computeReplacementTrivia)
			{
				...
				_visitStructuredTrivia = _nodeSet.Any((SyntaxNode n) => n.IsPartOfStructuredTrivia()) || _tokenSet.Any((SyntaxToken t) => t.IsPartOfStructuredTrivia()) || _triviaSet.Any((SyntaxTrivia t) => t.IsPartOfStructuredTrivia());
				_shouldVisitTrivia = _triviaSet.Count > 0 || _visitStructuredTrivia;
			}
		...
		private class BaseListEditor : VisualBasicSyntaxRewriter
		{
			private readonly TextSpan _elementSpan;

			protected readonly ListEditKind _editKind;
			...
			public override SyntaxTrivia VisitListElement(SyntaxTrivia element)
			{
				...
				if (_visitIntoStructuredTrivia && element.HasStructure && ShouldVisit(element.FullSpan))
				{
					result = base.VisitTrivia(element);
				}
				return result;
			}
		}
		...
		private class NodeListEditor : BaseListEditor
		{
			...
			private readonly IEnumerable<SyntaxNode> _replacementNodes;

			public NodeListEditor(SyntaxNode originalNode, IEnumerable<SyntaxNode> replacementNodes, ListEditKind editKind)
				: base(originalNode.FullSpan, editKind, visitTrivia: false, originalNode.IsPartOfStructuredTrivia())
			{
				...
				if (_originalNode is TNode)
				{
					...
					if (num >= 0 && num < list.Count)
					{
						...
						case ListEditKind.Replace:
							return list.ReplaceRange((TNode)_originalNode, _replacementNodes.Cast<TNode>());
						case ListEditKind.InsertBefore:
							return list.InsertRange(num, _replacementNodes.Cast<TNode>());
						case ListEditKind.InsertAfter:
							return list.InsertRange(num + 1, _replacementNodes.Cast<TNode>());
						}
					}
				}
		...
		private class TokenListEditor : BaseListEditor
		{
			...
			public TokenListEditor(SyntaxToken originalToken, IEnumerable<SyntaxToken> newTokens, ListEditKind editKind)
				: base(originalToken.FullSpan, editKind, visitTrivia: false, originalToken.IsPartOfStructuredTrivia())
			{
		...
		private class TriviaListEditor : BaseListEditor
		{
			...
			public TriviaListEditor(SyntaxTrivia originalTrivia, IEnumerable<SyntaxTrivia> newTrivia, ListEditKind editKind)
				: base(originalTrivia.FullSpan, editKind, visitTrivia: true, originalTrivia.IsPartOfStructuredTrivia())
			{
