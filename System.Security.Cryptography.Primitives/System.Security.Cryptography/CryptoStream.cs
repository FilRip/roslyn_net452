	...
	public class CryptoStream : Stream, IDisposable
	{
		...
		public CryptoStream(Stream stream, ICryptoTransform transform, CryptoStreamMode mode, bool leaveOpen)
		{
			...
			case CryptoStreamMode.Read:
				if (!_stream.CanRead)
				{
					throw new ArgumentException(SR.Format(SR.Argument_StreamNotReadable, "stream"));
				}
				_canRead = true;
				break;
			case CryptoStreamMode.Write:
				if (!_stream.CanWrite)
				{
					throw new ArgumentException(SR.Format(SR.Argument_StreamNotWritable, "stream"));
				}
				_canWrite = true;
				break;
			default:
		...
		private async Task<int> ReadAsyncCore(byte[] buffer, int offset, int count, CancellationToken cancellationToken, bool useAsync)
		{
			...
			if (_outputBufferIndex != 0)
			{
				...
				bytesToDeliver2 -= _outputBufferIndex;
			...
			if (_finalBlockTransformed)
			{
				return count - bytesToDeliver2;
			}
			int num = bytesToDeliver2 / _outputBlockSize;
			...
			while (bytesToDeliver2 > 0)
			{
				while (_inputBufferIndex < _inputBlockSize)
				{
					int num9 = ((!useAsync) ? _stream.Read(_inputBuffer, _inputBufferIndex, _inputBlockSize - _inputBufferIndex) : (await _stream.ReadAsync(new Memory<byte>(_inputBuffer, _inputBufferIndex, _inputBlockSize - _inputBufferIndex), cancellationToken)));
					int num3 = num9;
					if (num3 != 0)
					{
					...
					_finalBlockTransformed = true;
					if (bytesToDeliver2 < _outputBufferIndex)
					{
						Buffer.BlockCopy(_outputBuffer, 0, buffer, currentOutputIndex, bytesToDeliver2);
						_outputBufferIndex -= bytesToDeliver2;
						Buffer.BlockCopy(_outputBuffer, bytesToDeliver2, _outputBuffer, 0, _outputBufferIndex);
				...
				Buffer.BlockCopy(_outputBuffer, 0, buffer, currentOutputIndex, bytesToDeliver2);
				_outputBufferIndex = num8 - bytesToDeliver2;
				Buffer.BlockCopy(_outputBuffer, bytesToDeliver2, _outputBuffer, 0, _outputBufferIndex);
		...
		private async Task WriteAsyncCore(byte[] buffer, int offset, int count, CancellationToken cancellationToken, bool useAsync)
		{
			...
			{
				int numOutputBytes3 = _transform.TransformBlock(_inputBuffer, 0, _inputBlockSize, _outputBuffer, 0);
				if (useAsync)
				{
					await _stream.WriteAsync(new ReadOnlyMemory<byte>(_outputBuffer, 0, numOutputBytes3), cancellationToken);
				}
				else
				{
					_stream.Write(_outputBuffer, 0, numOutputBytes3);
				}
				_inputBufferIndex = 0;
			}
			while (bytesToWrite > 0)
			{
				if (bytesToWrite >= _inputBlockSize)
				{
					...
					if (_transform.CanTransformMultipleBlocks && num > 1)
					{
						...
						int numOutputBytes3 = 0;
						try
						{
							numOutputBytes3 = _transform.TransformBlock(buffer, currentInputIndex, numWholeBlocksInBytes, tempOutputBuffer, 0);
							if (useAsync)
							{
								await _stream.WriteAsync(new ReadOnlyMemory<byte>(tempOutputBuffer, 0, numOutputBytes3), cancellationToken);
							}
							else
							{
								_stream.Write(tempOutputBuffer, 0, numOutputBytes3);
							}
						...
						{
							CryptographicOperations.ZeroMemory(new Span<byte>(tempOutputBuffer, 0, numOutputBytes3));
							ArrayPool<byte>.Shared.Return(tempOutputBuffer);
						}
					}
					else
					{
						int numOutputBytes3 = _transform.TransformBlock(buffer, currentInputIndex, _inputBlockSize, _outputBuffer, 0);
						if (useAsync)
						{
							await _stream.WriteAsync(new ReadOnlyMemory<byte>(_outputBuffer, 0, numOutputBytes3), cancellationToken);
						}
