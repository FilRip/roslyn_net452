...
namespace Microsoft.CodeAnalysis.CSharp
{
    ...
    internal abstract class CSharpSemanticModel : SemanticModel
    {
        ...
        /// </summary>
        public new abstract CSharpSyntaxNode Root { get; }
        ...
        public TypeInfo GetTypeInfo(ExpressionSyntax expression, CancellationToken cancellationToken = default)
        {
            ...
            else if (SyntaxFacts.IsDeclarationExpressionType(expression, out DeclarationExpressionSyntax parent))
            {
                ...
                {
                    case SyntaxKind.SingleVariableDesignation:
                        var (declarationType, annotation) = TypeFromVariable((SingleVariableDesignationSyntax)parent.Designation, cancellationToken);
        ...
        protected int CheckAndAdjustPosition(int position, out SyntaxToken token)
        {
            ...
            throw new ArgumentOutOfRangeException(nameof(position), position,
                string.Format(Properties.Resources.PositionIsNotWithinSyntax, Root.FullSpan));
        }
        ...
        protected void CheckSyntaxNode(CSharpSyntaxNode syntax)
        {
            ...
            if (!IsInTree(syntax))
            {
                throw new ArgumentException(Properties.Resources.SyntaxNodeIsNotWithinSynt);
            }
        }
        ...
        private void CheckModelAndSyntaxNodeToSpeculate(CSharpSyntaxNode syntax)
        {
            ...
            {
                throw new InvalidOperationException(Properties.Resources.ChainingSpeculativeModelIsNotSupported);
            }

            if (this.Compilation.ContainsSyntaxTree(syntax.SyntaxTree))
            {
                throw new ArgumentException(Properties.Resources.SpeculatedSyntaxNodeCannotBelongToCurrentCompilation);
            }
        }
        ...
        {
            if (s.OriginalDefinition is ErrorTypeSymbol originalErrorSymbol)
            {
        ...
        internal ImmutableArray<IPropertySymbol> GetIndexerGroupForNode(
            ...
            Binder binderOpt)
        {
            if (lowestBoundNode is BoundExpression boundExpr && boundExpr.Kind != BoundKind.TypeExpression)
            {
        ...
        /// <param name="cancellationToken">The cancellation token.</param>
        /// <returns>The symbol that was declared.</returns>
        public abstract ISymbol GetDeclaredSymbol(LocalFunctionStatementSyntax declarationSyntax, CancellationToken cancellationToken = default);
                ...
                case BoundKind.TypeExpression:
                    {
                        ...
                        if (typeSymbol.OriginalDefinition is ErrorTypeSymbol originalErrorType)
                        {
        ...
        private static ImmutableArray<MethodSymbol> FilterOverriddenOrHiddenMethods(ImmutableArray<MethodSymbol> methods)
        {
            ...
            HashSet<Symbol> hiddenSymbols = new();
        ...
        private SymbolInfo GetNamedArgumentSymbolInfo(IdentifierNameSyntax identifierNameSyntax, CancellationToken cancellationToken)
        {
            ...
            if (parent3.IsKind(SyntaxKind.TupleExpression))
            {
                ...
                return tupleElement == null ? SymbolInfo.None : new SymbolInfo(tupleElement, ImmutableArray<ISymbol>.Empty, CandidateReason.None);
            }
            ...
            if (containingInvocationInfo.Symbol != null)
            {
        ...
        private static ImmutableArray<MethodSymbol> CreateReducedExtensionMethodsFromOriginalsIfNecessary(BoundCall call, CSharpCompilation compilation)
        {
            ...
            {
                AddReducedAndFilteredMethodGroupSymbol(methodBuilder, filteredMethodBuilder, method, default, extensionThisType, compilation);
            }
            methodBuilder.Free();
        ...
        protected sealed override DataFlowAnalysis AnalyzeDataFlowCore(SyntaxNode statementOrExpression)
        {
            return statementOrExpression switch
            {
                ...
                _ => throw new ArgumentException("statementOrExpression is not a StatementSyntax or an ExpressionSyntax."),
            };
        }
